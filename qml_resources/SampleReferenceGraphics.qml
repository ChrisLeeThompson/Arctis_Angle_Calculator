...
// and each beam line grows inward from the arc. This is the Hydra
// milling-figure layout adapted to the Arctis Compustage: there is no stage
// rotation regime, so the chip list for a beam is simply every achievable
...
//    reads at a glance. The border is dim at rest and brightens, with a
//    translucent relation-colored fill, when the stage is currently AT that
//    position (the chip's "active" state); the number takes the relation
//    color when active. Hovering a chip shows a tooltip with the relation,
//    milling angle, and alpha tilt for that position. Clicking a chip applies
//    that position directly.
//
// The chip lists come from MillingAngleCalculations.achievableTiltsForBeam().
// Per-beam cycle indices reset whenever the chalk lines change.
...
Item {
    ...
    Canvas {
        ...
        onPaint: {
            ...
            for (var i = 0; i < beams.length; i++) {
                ...
                if (beams[i] === "FIB") {
                    // FIB extends across the full diameter, reaching the arc on
                    // the far side too (matches the previous UI). Drawn as two
                    // center-out radials so it shares drawRadialLine's angle
                    // convention.
    ...
    Repeater {
        ...
        delegate: Label {
            ...
            // Chips sit on the outer side of the label: to the left for the
            // left-leaning beams and SEM (angle >= 90), to the right for any
            // right-leaning beam. Keeps the chip row off the center of the
            // figure.
            ...
            // left-side beams); each new row stacks below the previous.
            Grid {
                id: chipRow
